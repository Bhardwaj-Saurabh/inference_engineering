...
This repository is currently an empty `uv`-managed Python scaffold for a personal 12-week learning project on LLM inference engineering (see `docs/plan/12-week-llm-inference-engineering-plan-v2.md`). There is no application code yet beyond the default `main.py` generated by `uv init`. As the plan is executed, expect a portfolio of numbered sub-projects (e.g. `01-inference-from-scratch/`, `02-gpu-kernels-and-profiling/`, `06-mini-inference-engine/`, ...) to be added under this root — check `docs/plan/` for the authoritative structure before assuming any layout.
